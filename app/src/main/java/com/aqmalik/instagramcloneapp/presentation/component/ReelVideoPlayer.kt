...
fun ReelVideoPlayer(uri: Uri) {
    ...
    Log.d("VIDEOURI", "ReelVideoPlayer: $uri")
    ...
    // AndroidView to integrate ExoPlayer with PlayerView
    AndroidView(
        factory = {
            PlayerView(context).apply {
                hideController() // Hide playback controls
                useController = false // Disable playback controls
                resizeMode = AspectRatioFrameLayout.RESIZE_MODE_ZOOM // Adjust aspect ratio
                player = exoPlayer
                layoutParams = FrameLayout.LayoutParams(MATCH_PARENT, MATCH_PARENT)
            }
        },
        modifier = Modifier.fillMaxSize() // Ensure it takes up the full size
    )
